...
func NewPollardJumper(k, c, p *big.Int) (*PollardJumper, error) {
	if k.Sign() != 1 || c.Sign() != 1 || p.Sign() != 1 {
		return nil, fmt.Errorf("k, c, p must be positive: %v, %v, %v", k, c, p)
	}
	return &PollardJumper{k: k, c: c, p: p, two: big.NewInt(2)}, nil
}

func (p PollardJumper) F(y, dst *big.Int) {
	dst.Exp(p.two, dst.Mod(y, p.k), p.p) // dst = 2^(y mod k) mod p
}

func (p PollardJumper) N() *big.Int {
	// No need to pre-allocate, since this isn't a hot path.
	one := big.NewInt(1)

	// Will hold all possible outputs of F.
	seen := make([]*big.Int, p.k.Int64())
	// for i in [0, k)
...
func PollardsKangaroo(p, g, a, b, y *big.Int, pj PollardJumper) (*big.Int, error) {
	var (
		// Tame kangaroo
		xT, yT big.Int
		// Wild kangaroo
